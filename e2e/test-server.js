...
function buildMockText(prompt) {
  ...
  if (prompt.includes('一期讲透 AI 视频笔记')) {
    return [
      '## 视频核心结论',
      '- 讲者用「抓字幕 → 取结构 → 留行动项」三步，把一小时的视频压成三分钟笔记。',
      '- 视频笔记的价值在于可检索：字幕原文、时间戳和要点互相跳转。',
      '',
      '## 字幕要点',
      '- 00:00 为什么视频学习容易「看过就忘」：缺少结构与回看入口。',
      '- 06:24 字幕是现成底稿：AI 摘要先读字幕，再按主题重组要点。',
      '- 21:40 行动项先行：每条结论都应落到一条可执行动作。',
      '- 38:12 用本地历史沉淀视频笔记，按频道与主题检索。',
      '',
      '## 适合谁看',
      '- 需要反复消化课程、访谈与技术分享内容的学习者。'
    ].join('\n');
  }
